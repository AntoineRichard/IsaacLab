...
* Added the contributed MicroDuck gear-backlash task ``IsaacContrib-Velocity-Flat-MicroDuck-Backlash``
  ...
  velocity PPO configuration, as upstream does, and differs from the base flat task in exactly three
  places: the robot, the encoder view in both observation groups, and the solver's constraint budget.
  ``dof_pos_limits`` is scoped to the 14 servos, since a play hinge rides its limits by construction
  and an unscoped soft-limit penalty would be a permanent tax; the armature randomization
...
* Sized the MuJoCo Warp constraint budget of the backlash task from profiling rather than inheriting
  ...
  256 environments and 66 at 2048, under random actions with the tilt termination dropped. The
  structural bound is the base task's 54 plus one permanently active limit row per play hinge, and
  the base task's shipped 64 is below the measured peak. ``nconmax`` is unchanged at 10: the play
  hinges are joints, not colliders.
