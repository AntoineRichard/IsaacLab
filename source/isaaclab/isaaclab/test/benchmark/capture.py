...
def capture_resources(bm: Any) -> Resources:
    ...
    Peak values are read from ``"<metric> peak"`` rows.  When a peak row is
    absent the mean is used as the peak (avoiding the
    :class:`~.schema.MeanStd` post-init ``ValueError``).  Utilisation fields
    (:attr:`~.schema.Resources.gpu_util_pct`,
    :attr:`~.schema.Resources.cpu_util_pct`) always have ``peak=None`` because
    a ceiling of 100% is uninformative.
