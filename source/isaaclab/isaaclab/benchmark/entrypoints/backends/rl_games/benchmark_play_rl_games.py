...
Rolls out a checkpointed RL-Games policy under a :class:`~isaaclab.benchmark.BenchmarkMonitor`
and emits a :class:`~isaaclab.benchmark.schema.PlayBundle` JSON file. Dispatched from
``isaaclab benchmark play`` via ``--rl_library rl_games``.
"""
...
def run(argv: list[str]) -> BenchmarkResult:
    """Run the RL-Games play benchmark and write a :class:`~isaaclab.benchmark.schema.PlayBundle`.
