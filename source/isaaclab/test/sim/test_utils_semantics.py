...
from isaaclab.sim.utils.semantics import add_labels, check_missing_labels, count_total_labels, get_labels, remove_labels
...
def create_test_environment_with_labels():
    ...
    for i in range(3):
        create_prim(f"/World/Test/Object{i}", "Cube", semantic_label="cube")
    # create a sphere without any labels
    create_prim("/World/Test/Object3", "Sphere")
    # create a nested prim with label "nested"
    nested_prim = create_prim("/World/Test/Object0/Nested", "Cube")
    add_labels(nested_prim, ["nested"], instance_name="shape")
...
def test_add_and_get_labels():
    ...
    # Apply semantics
    add_labels(prim, ["label_a", "label_b"], instance_name="class")
    add_labels(prim, ["shape_a"], instance_name="shape")
    add_labels(nested_prim, ["nested_label"], instance_name="class")
...
def test_add_labels_with_overwrite():
    ...
    add_labels(prim, ["replaced_label"], instance_name="class", overwrite=True)
    labels_dict = get_labels(prim)
...
def test_add_labels_without_overwrite():
    ...
    add_labels(prim, ["label_c"], instance_name="class", overwrite=False)
    labels_dict = get_labels(prim)
...
def test_remove_specific_labels():
    ...
    add_labels(prim, ["shape_a"], instance_name="shape")
    add_labels(nested_prim, ["nested_label"], instance_name="class")
    add_labels(nested_prim, ["nested_shape"], instance_name="shape")
    ...
    # Remove specific labels
    remove_labels(prim, instance_name="class", include_descendants=True)
    # Check labels are removed correctly
    labels_dict = get_labels(prim)
    assert len(labels_dict) == 0
    # Check nested prim labels are removed
...
def test_check_missing_labels():
    ...
    missing_paths_subtree = check_missing_labels(prim_path="/World/Test/Object0")
    # Object0 and Nested both have labels
    assert len(missing_paths_subtree) == 0

    # Check from invalid path
    missing_paths_invalid = check_missing_labels(prim_path="/World/Test/Invalid")
...
def test_count_labels_in_scene():
    ...
    # Count from root
    labels_dict = count_total_labels()
    # Object0 and Nested both have labels
    ...
    labels_dict_subtree = count_total_labels(prim_path="/World/Test/Object0")
    ...
    labels_dict_invalid = count_total_labels(prim_path="/World/Test/Invalid")
