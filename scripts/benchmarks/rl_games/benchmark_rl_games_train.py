...
def _parse_args(argv: list[str]):
    ...
    Returns:
        ...
        preset tokens retained for the 2.x Hydra configuration lookup.
    """
    import argparse
...
def _apply_training_overrides(args_cli, env_cfg, agent_cfg):
    """Apply Lab 2 environment and RL-Games training overrides."""
    import math
    import random
    ...
    batch_size = env_cfg.scene.num_envs * config.get("horizon_length", 16)
    minibatch_size = config.get("minibatch_size")
    if minibatch_size is not None and batch_size % minibatch_size != 0:
        config["minibatch_size"] = math.gcd(batch_size, minibatch_size)
...
def run(argv: list[str]) -> None:
    ...
    try:
        ...
        agent_cfg = _apply_training_overrides(args_cli, env_cfg, agent_cfg)
