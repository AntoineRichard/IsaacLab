...
"""Smoke test for scripts/benchmarks/training.py with --rl_library rl_games."""
...
def test_training_rl_games_writes_training_bundle(tmp_path, require_isaacsim):
    sh = ROOT / "isaaclab.sh"
    cmd = [
        str(sh),
        "-p",
    ...
    assert data["run"]["framework"] == "rl_games"
