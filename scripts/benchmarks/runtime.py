...
This 2.x compatibility script supports PhysX. It accepts the 3.0
``presets=physx`` spelling and rejects 3.0-only solver presets before launch.
...
try:
    from scripts.benchmarks._compat import (
        add_launcher_args,
        apply_env_overrides,
        launch_app,
    ...
    from _compat import (  # type: ignore[no-redef]
        add_launcher_args,
        apply_env_overrides,
        launch_app,
        parse_benchmark_args,
...
def _parse_args(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    return parse_benchmark_args(parser, argv)
