...
We need to run benchmarked trainings, with different seeds, and collect the results. The naming of these trainings should be transparent i.e. if training ant, let's not have a hash or just ant. But maybe BACKEND\_TASK\_DATE\_SEED. Or something like that. Setting this up should require minimum work, as almost all the elements should be there. Our main interests lie in runtime perf, start-up perf, system resources consumption, and reward reached.
...
When a job is done the monitor should pull the results.
